  ...
  const renderVehicles = () => ( 
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {vehicles.filter(v => v.plateNumber.toLowerCase().includes(searchTerm.toLowerCase()) || v.driverName.toLowerCase().includes(searchTerm.toLowerCase())).map(v => {
              ...
              const isDispatchLocked = vehicleTrips.length > 0;
              const activeTrip = vehicleTrips.find(t => t.status !== TripStatus.COMPLETED);
              ...
              <div key={v.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 hover:shadow-md transition">
                  ...
                  <div className="flex justify-between items-start mb-3">
                      ...
                      <div className="flex gap-1">
                          ...
                          <button onClick={() => handleOpenVehicleModal(v)} className="p-1.5 hover:bg-slate-100 rounded text-slate-500 hover:text-brand-600" title="Sửa thông tin xe"><Edit2 className="w-4 h-4"/></button>
                          {!isDispatchLocked && (
                            <button
