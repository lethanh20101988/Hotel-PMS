...
export const DeleteFundModal: React.FC<DeleteFundModalProps> = ({ transaction, onClose, onConfirm }) => {
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden animate-fade-in">
        <div className="bg-red-600 p-4 text-white flex justify-between items-center">
          <h3 className="font-bold flex items-center gap-2">
            <Trash2 className="w-5 h-5" /> Xóa phiếu?
          </h3>
          <button onClick={onClose} className="hover:bg-red-700 p-1 rounded"><X className="w-5 h-5" /></button>
        </div>
        <div className="p-6 text-center space-y-4">
           <AlertTriangle className="w-12 h-12 text-red-500 mx-auto" />
           <p className="text-slate-600">Bạn có chắc chắn muốn xóa phiếu <b>{transaction.id}</b>?</p>
           <p className="text-xs text-slate-400">Hành động này sẽ xóa cả bút toán sổ nhật ký chung tương ứng.</p>
        </div>
