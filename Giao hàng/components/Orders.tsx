...
const Orders: React.FC<Props> = ({ orders, distributors, products, trips, warehouseReceipts = [], onAdd, onUpdate, onDelete }) => {
  ...
  const handleImportSubmit = (e: React.FormEvent) => {
    ...
    // Do chúng ta không có backend để đọc file thực tế, 
    // đoạn code này sẽ giả lập việc đọc dữ liệu từ file mẫu và thêm vào hệ thống.
    
    const today = new Date().toISOString().split('T')[0];
    const currentMonth = today.substring(0, 7);
    const timestamp = Date.now();

    // 1. Giả lập Đơn hàng 1: Giao cho NPP (tương ứng dòng 1 trong file mẫu)
    // Lấy NPP đầu tiên hoặc NPP01 nếu có
    const dist1 = distributors.find(d => d.id === 'NPP01') || distributors[0];
    const prod1 = products.find(p => p.id === 'SP01') || products[0];
    const prod2 = products.find(p => p.id === 'SP02') || products[1];

    if (dist1 && prod1 && prod2) {
        const order1Items = [
            { productId: prod1.id, productName: prod1.name, quantity: 100 },
            { productId: prod2.id, productName: prod2.name, quantity: 50 }
        ];
        
        const weight1 = order1Items.reduce((sum, item) => {
        ...
        const order1: Order = {
            ...
            totalQuantity: 150,
            totalWeight: weight1,
            note: 'Import từ Excel: Giao gấp sáng sớm',
            status: OrderStatus.CREATED
        };
        onAdd(order1);
    }

    // 2. Giả lập Đơn hàng 2: Giao nội bộ (tương ứng dòng 2 trong file mẫu)
    const prod3 = products.find(p => p.id === 'SP03') || products[2] || products[0];
    if (prod3) {
        const order2Items = [
            { productId: prod3.id, productName: prod3.name, quantity: 200 }
        ];
        
        const weight2 = order2Items.reduce((sum, item) => {
            const p = products.find(prod => prod.id === item.productId);
            return sum + (item.quantity * (p?.weightKg || 0));
        }, 0);
        ...
        const order2: Order = {
            ...
            distributorName: 'Kho Đà Nẵng', // Display name for internal
            fromWarehouse: 'Kho Tổng Huế',
            requestDate: today,
            month: currentMonth,
            items: order2Items,
            totalQuantity: 200,
            totalWeight: weight2,
            note: 'Import từ Excel: Điều chuyển hàng tồn',
            status: OrderStatus.CREATED
        };
        onAdd(order2);
    }

    alert(`Đã xử lý file "${importFile.name}" thành công!\nĐã thêm 2 đơn hàng mới vào hệ thống.`);
  ...
  const handleDownloadTemplate = (e: React.MouseEvent) => {
      e.preventDefault();
      // Define CSV content detailed based on View Order structure
      // Support grouping multiple items into one order via "Ma_Don_Tham_Chieu"
      const headers = [
          "Ma_Don_Tham_Chieu(Gom_Nhom_SP)", // Col 1: Mã dùng để gom nhiều dòng thành 1 đơn
          "Loai_Don(NPP/NOIBO)",            // Col 2
          "Ma_NPP_Hoac_Kho_Dich",           // Col 3
          "Kho_Xuat_Hang",                  // Col 4
          "Ngay_Yeu_Cau(YYYY-MM-DD)",       // Col 5
          "Ma_San_Pham",                    // Col 6
          "So_Luong",                       // Col 7
          "Ghi_Chu_Don_Hang"                // Col 8
      ];

      // Sample Data:
      // Order 1: Delivered to NPP01, contains 2 products (SP01, SP02) -> Takes 2 rows
      const row1_1 = ["IMP-001", "NPP", "NPP01", "Kho Tổng Huế", "2023-12-01", "SP01", "100", "Giao gấp sáng sớm"];
      const row1_2 = ["IMP-001", "NPP", "NPP01", "Kho Tổng Huế", "2023-12-01", "SP02", "50", "Giao gấp sáng sớm"];
      
      // Order 2: Internal Transfer to Da Nang, 1 product
      const row2 = ["IMP-002", "NOIBO", "Kho Đà Nẵng", "Kho Tổng Huế", "2023-12-05", "SP03", "200", "Điều chuyển hàng tồn"];
      
      const csvContent = "\uFEFF" + [headers.join(","), row1_1.join(","), row1_2.join(","), row2.join(",")].join("\n");
